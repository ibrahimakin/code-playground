...
const resultFrame = document.getElementById('code-result');
resultFrame.appendChild(iframeOM);                          // this can not apply to curent div
const codeResult = resultFrame.children[0];

const replaceChar = (innerHtml) => { return innerHtml.replace(/<div>/g, '').replace(/<\/div>/g, '').replace(/&lt;/g, '<').replace(/&gt;/g, '>'); }
...
const highLite = el => {
  ...
  let contentDOM = '<html><body>' + htmlResult + '<script type="text/javascript">' + scriptResult + '</script></body></html>';
  codeResult.src = 'data:text/html;charset=utf-8,' + escape(contentDOM);
  // let docu = iframeOM.contentWindow.document; docu.open(); docu.writeln(contentDOM); docu.close();
};
